...
class AssetManager:
    ...
    def _bootstrap_mock_content(self):
        ...
        for i in range(3000):
            a = AssetMetadata(
                f"Sound {i}", AssetType.SFX, "Combat", tags=["sword", "clash", "metal"]
            )
            self._assets[a.uuid] = a
    ...
    def search(
        ...
        for asset in self._assets.values():
            ...
            if q:
                match = q in asset.name.lower() or any(
                    q in tag.lower() for tag in asset.tags
                )
                if not match:
                    continue
    ...
    def toggle_favorite(self, asset_uuid: str):
        if asset_uuid in self._assets:
            self._assets[asset_uuid].is_favorite = not self._assets[
                asset_uuid
            ].is_favorite
