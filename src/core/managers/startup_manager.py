...
class StartupManager:
    ...
    def boot(self, splash=None):
        ...
        while True:
            ...
            box.setText(msg)
            box.setIcon(QMessageBox.Critical if report["errors"] else QMessageBox.Warning)
            
            btn_retry = box.addButton("Retry", QMessageBox.ActionRole)
        ...
        registry.register_factory(AIManager, lambda: AIManager(registry.get(EventBus), registry.get(ConfigurationManager)))
        registry.register_factory(ZanimeAIAPI, lambda: ZanimeAIAPI(registry.get(AIManager)))
