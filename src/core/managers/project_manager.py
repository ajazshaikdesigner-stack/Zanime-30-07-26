...
from PySide6.QtCore import QTimer, QObject, QFileSystemWatcher
...
class ProjectManager(QObject):
    def __init__(self, event_bus: EventBus):
        ...
        self.project_model: Optional[ProjectModel] = None
    ...
    def _enforce_structure(self, extract_path: str):
        directories = [
            ...
            "storyboard", "scenes", "timeline", "voice", "music", 
            "render", "cache", "autosave", "logs", "exports", "assets", "thumbnails"
        ]
    ...
    def _check_recovery(self, extract_path: str):
        ...
        if os.path.exists(autosave_path) and os.path.exists(project_json_path):
            if os.path.getmtime(autosave_path) > os.path.getmtime(project_json_path):
                logger.warning("Autosave is newer than project.json. Recovery might be needed.")
    ...
    def autosave(self) -> None:
        ...
        project_name = os.path.basename(self.current_project_path).replace(".zanime", "")
        extract_path = os.path.join(self.temp_dir, project_name)
    ...
    def save_project(self) -> None:
        ...
        with zipfile.ZipFile(self.current_project_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for root, dirs, files in os.walk(extract_path):
                ...
                if "autosave" in dirs: dirs.remove("autosave")
