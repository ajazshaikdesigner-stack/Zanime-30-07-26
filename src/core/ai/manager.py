...
class AIManager(QObject):
    ...
    def unload_idle_models(self):
        ...
        if self.active_provider and (time.time() - self.last_activity_time > 300):
            logger.info(f"AI Provider {self.active_provider} has been idle. Unloading to free VRAM.")
    ...
    def _enforce_vram_limits(self, incoming_provider_type: str) -> bool:
        ...
        if self.active_provider and self.active_provider != incoming_provider_type:
            logger.warning(f"Unloading {self.active_provider} to free VRAM for {incoming_provider_type}")
            self.providers[self.active_provider].unload()
            self.event_bus.publish(Event.AI_MODEL_UNLOADED, self.active_provider)
            self.active_provider = None
        return True
        
    def execute_task(self, provider_type: str, model_name: str, prompt: str, params: Dict[str, Any], priority: int = 0) -> str:
        ...
        if not provider.is_loaded:
            if not provider.load(model_name, self.config_manager.get_user("ai_settings", {})):
                raise RuntimeError(f"Failed to load model {model_name} for {provider_type}")
        ...
        task.signals.started.connect(lambda t_id: self.event_bus.publish(Event.AI_TASK_STARTED, t_id))
        task.signals.progress.connect(lambda t_id, p, m: self.event_bus.publish(Event.AI_TASK_PROGRESS, {"id": t_id, "progress": p, "msg": m}))
        task.signals.finished.connect(lambda t_id, res: self.event_bus.publish(Event.AI_TASK_COMPLETED, {"id": t_id, "result": res}))
        task.signals.error.connect(lambda t_id, err: self.event_bus.publish(Event.AI_TASK_FAILED, {"id": t_id, "error": err}))
