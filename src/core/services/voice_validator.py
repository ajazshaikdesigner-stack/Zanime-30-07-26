        ...
        for track in timeline.tracks:
            ...
            for i in range(len(clips_sorted) - 1):
                ...
                if current.start_frame + current.duration > next_clip.start_frame:
                    errors.append(
                        f"Overlapping Audio: '{current.text}' overlaps with next clip."
                    )

            for clip in track.clips:
                if not clip.voice_profile_uuid:
                    errors.append(
                        f"Missing Voice: Clip starting at {clip.start_frame} has no assigned voice."
                    )
                if not clip.text and not clip.audio_path:
                    errors.append(
                        f"Missing Dialogue: Clip starting at {clip.start_frame} has no text or audio."
                    )
