        ...
        for track in timeline.tracks:
            for clip in track.clips:
                if clip.duration < rapid_cut_threshold:
                    errors.append(f"Rapid Cut: Clip '{clip.name}' is too short ({clip.duration} frames).")
                ...
                if clip.transition_in == "Custom Transition":
                    errors.append(f"Validation Warning: '{clip.name}' uses undefined Custom Transition.")
