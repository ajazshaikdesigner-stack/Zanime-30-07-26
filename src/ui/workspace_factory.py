...
class WorkspaceErrorWidget(QWidget):
    def __init__(self, name: str, exc: Exception, parent=None):
        ...
        lbl_err = QLabel(f"{type(exc).__name__}: {str(exc)}")
    ...
    def get_required_docks(self): return []
    def get_hidden_docks(self): return []
...
class WorkspaceFactory:
    ...
    def get_workspace(self, name: str, app, main_window) -> QWidget:
        ...
        try:
            ...
            logger.error(f"WorkspaceFactory: Crash during {name} initialization: {e}", exc_info=True)
