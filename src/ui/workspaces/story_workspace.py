...
class StoryWorkspace(BaseWorkspace):
    def __init__(self, app, parent=None):
        ...
        self.editor.action_rewrite.triggered.connect(lambda: self._generate_text("Rewrite"))
        self.editor.action_expand.triggered.connect(lambda: self._generate_text("Expand"))
        self.editor.action_continue.triggered.connect(lambda: self._generate_text("Continue"))
        
        self.settings_dock.generate_btn.clicked.connect(lambda: self._generate_text("Generate New Story"))
    ...
    def _generate_text(self, action: str):
        ...
        if action == "Generate New Story":
            ...
            full_prompt = f"{action} this text: {self.editor.text_edit.toPlainText()[:500]}"
    ...
    def autosave_story(self):
        if registry.get(ProjectManager).current_project_path:
            ...
            project_name = os.path.basename(pm.current_project_path).replace(".zanime", "")
            story_path = os.path.join(pm.temp_dir, project_name, "story", "story_data.json")
    ...
    def lock_story(self):
        ...
        QMessageBox.information(self, "Story Locked", "This story has been locked for future phases.")
