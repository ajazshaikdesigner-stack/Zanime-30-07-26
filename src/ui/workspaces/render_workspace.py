...
from src.models.render_model import RenderJob, RenderSettings
...
class RenderWorkspace(BaseWorkspace):
    ...
    def _on_queue_render(self):
        ...
        self.queue_dock.list.addItem(f"{job.name} ({job.settings.resolution}, {job.settings.fps}fps) - QUEUED")
