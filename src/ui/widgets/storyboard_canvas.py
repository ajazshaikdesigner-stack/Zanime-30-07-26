    ...
    def render_board(self, storyboard_model):
        ...
        for scene in storyboard_model.scenes:
            for shot in scene.shots:
                lbl = QLabel(f"<b>Scene {scene.number}</b> - Shot {shot.number}<br>{shot.shot_type}<br>{shot.duration}s")
                lbl.setAlignment(Qt.AlignCenter)
                lbl.setStyleSheet("border: 2px solid #555; background-color: #333; padding: 10px;")
