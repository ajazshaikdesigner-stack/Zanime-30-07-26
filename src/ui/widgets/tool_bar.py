...
class ZanimeToolBar(QToolBar):
    ...
    def _setup_tools(self):
        ...
        if new_icon and not new_icon.isNull(): self.new_action.setIcon(new_icon)
        
        self.save_action = self.addAction("Save")
        if save_icon and not save_icon.isNull(): self.save_action.setIcon(save_icon)
        self.save_action.triggered.connect(self._on_save)
        
        self.addSeparator()

        self.undo_action = self.addAction("Undo")
        if undo_icon and not undo_icon.isNull(): self.undo_action.setIcon(undo_icon)
        self.undo_action.triggered.connect(self._on_undo)

        self.redo_action = self.addAction("Redo")
        if redo_icon and not redo_icon.isNull(): self.redo_action.setIcon(redo_icon)
